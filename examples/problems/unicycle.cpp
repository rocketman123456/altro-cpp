...
namespace altro
{
    namespace problems
    {
        ...
        altro::problem::Problem UnicycleProblem::MakeProblem(const bool add_constraints)
        {
            ...
            for (int k = 0; k < N; ++k)
            {
                qcost = std::make_shared<examples::QuadraticCost>(examples::QuadraticCost::LQRCost(Q, R, xf, uref));
                prob.SetCostFunction(qcost, k);
            }
            qterm = std::make_shared<examples::QuadraticCost>(examples::QuadraticCost::LQRCost(Qf, R * 0, xf, uref, true));
