...
namespace altro
{
    namespace examples
    {
        ...
        void QuadraticCost::Gradient(const VectorXdRef& x, const VectorXdRef& u, Eigen::Ref<VectorXd> dx, Eigen::Ref<VectorXd> du)
        {
            dx = Q_ * x + q_ + H_ * u;
            du = R_ * u + r_ + H_.transpose() * x;
        }

        void QuadraticCost::Hessian(const VectorXdRef& x, const VectorXdRef& u, Eigen::Ref<MatrixXd> dxdx, Eigen::Ref<MatrixXd> dxdu, Eigen::Ref<MatrixXd> dudu)
        {
            ALTRO_UNUSED(x);
            ALTRO_UNUSED(u);
            dxdx = Q_;
