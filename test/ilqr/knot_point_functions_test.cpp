    ...
    namespace ilqr
    {
        ...
        protected:
            ...
            KnotPointFunctions<n_size, m_size> MakeKPF()
            {
                ...
                std::shared_ptr<examples::QuadraticCost> costfun_ptr = std::make_shared<examples::QuadraticCost>(costfun);
            ...
            void TestGains(double rho = 0)
            {
                ...
                kpf.CalcGains();
                MatrixXd K = (Quu + MatrixXd::Identity(m, m) * rho).ldlt().solve(Qxu.transpose());
            ...
            void TestCTG()
            {
                ...
                MatrixXd Sxx    = Qxx + K.transpose() * Quu * K + K.transpose() * Qux + Qxu * K;
        ...
        TEST_F(KnotPointFunctionsTest, ConstructionDeath)
        {
            ...
            {
                auto null_dynamics = [&]() { KnotPointFunctions<n_static, m_static> kpf(nullptr, costfun_ptr); };
                EXPECT_DEATH(null_dynamics(), "Assert.*null dynamics pointer");

                auto null_costfun = [&]() { KnotPointFunctions<n_static, m_static> kpf(model_ptr, nullptr); };
        ...
        {
            // Create random data
            KnotPoint<n_static, m_static> z_static = KnotPoint<n_static, m_static>::Random();
            ...
            KnotPointFunctions<n_static, m_static> kpf_static  = MakeKPF<n_static, m_static>();
            ...
            double J_expected = 0.5 * (x.dot(Q * x) + u.dot(R * u)) + q.dot(x) + r.dot(u) + c;
        ...
        TEST_F(KnotPointFunctionsTest, CostExpansion)
        {
            ...
            CostExpansion<n_static, m_static> cost_expansion = kpf_static.GetCostExpansion();
        ...
        TEST_F(KnotPointFunctionsTest, DynamicsExpansion)
        {
            ...
            DynamicsExpansion<n_static, m_static> dyn_exp = kpf_static.GetDynamicsExpansion();
            MatrixXd                              jac(n, n + m);
        ...
        TEST_F(KnotPointFunctionsTest, TerminalCostToGoStatic) { TestTermCTG<n_static, m_static>(); }

        TEST_F(KnotPointFunctionsTest, TerminalCostToGoDynamic) { TestTermCTG<HEAP, HEAP>(); }

        TEST_F(KnotPointFunctionsTest, ActionValueExpansionStatic) { TestQExpansion<n_static, m_static>(); }

        TEST_F(KnotPointFunctionsTest, ActionValueExpansionDynamic) { TestQExpansion<HEAP, HEAP>(); }
        ...
        TEST_F(KnotPointFunctionsTest, CalcCostToGoStatic) { TestCTG<n_static, m_static>(); }
