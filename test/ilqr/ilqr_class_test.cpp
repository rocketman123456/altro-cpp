...
namespace altro
{
    namespace ilqr
    {
        ...
        TEST(iLQRClassTest, ReferenceCounts)
        {
            ...
            ptrs.emplace_back(std::make_unique<ilqr::KnotPointFunctions<HEAP, HEAP>>(model, costfun));
            ...
            ptrs.push_back(std::make_unique<ilqr::KnotPointFunctions<HEAP, HEAP>>(model, costfun));
            EXPECT_EQ(model.use_count(), 3);
        ...
        TEST(iLQRClassTest, DeathTests)
        {
            ...
            {
                EXPECT_DEATH(ilqr.CopyFromProblem(prob_undefined, 0, N + 1), "Assert.*fully defined");
            }
