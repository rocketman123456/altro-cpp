...
namespace altro
{
    namespace benchmarks
    {
        ...
        void RunTest(ilqr::iLQR<Nx, Nu>& solver, Time tserial, int Nruns, int nthreads, int tasks_per_thread = 1, FILE* io = stdout)
        {
            ...
            fmt::print(
                io,
                "Parallel w/ {} tasks and {} threads: {:.2f}x speedup, {:.2f}% expected\n",
                solver.NumTasks(),
                solver.NumThreads(),
                tserial / tparallel,
...
int main() { altro::benchmarks::ParallelExpansions(); }