...
namespace altro
{
    ...
    std::string SolverStats::ProfileOutputFile() { return opts_.log_directory + "/" + opts_.profile_filename; }
    ...
    void SolverStats::DefaultLogger()
    {
        ...
        SetPtr(
            logger_.AddEntry(-1, "reg", "{:>.1e}").SetName("regularization").SetLevel(LogLevel::kInnerDebug).SetWidth(7), // NOLINT(readability-magic-numbers)
            regularization
        );
        SetPtr(
            logger_.AddEntry(-1, "z", "{:>.3f}")
        ...
        SetPtr(
            logger_.AddEntry(-1, "pen", "{:>.1e}").SetName("max_penalty").SetLevel(LogLevel::kDebug).SetWidth(7), // NOLINT(readability-magic-numbers)
            max_penalty
        );
