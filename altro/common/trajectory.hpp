...
namespace altro
{
    ...
    public:
         ...
         */
        explicit Trajectory(int N)
            : traj_(N + 1)
        {}
        explicit Trajectory(int _n, int _m, int N)
            : traj_(N + 1, KnotPoint<n, m, T>(_n, _m))
        {}
        explicit Trajectory(std::vector<KnotPoint<n, m, T>> zs)
            : traj_(zs)
        {}
        ...
        /***************************** Moving ***************************************/
        Trajectory(Trajectory&& Z) noexcept
            ...
            for (int k = 0; k < NumSegments(); ++k)
            {
                ...
                if (std::abs(h_stored - h_calc) > eps)
                {
                    if (verbose)
                    {
                        ...
                        std::cout << "t-=" << GetTime(k) << "\t t+=" << GetTime(k + 1) << "\t dt=" << h_calc << std::endl;
                    }
                    return false;
                }
            }
