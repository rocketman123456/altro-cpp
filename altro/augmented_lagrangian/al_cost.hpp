    ...
    namespace augmented_lagrangian
    {
        ...
        public:
            ...
            ALCost(const problem::Problem& prob, const int k)
                : n_(prob.GetDynamics(k)->StateDimension())
                , m_(prob.GetDynamics(k)->ControlDimension())
            {
                SetCostFunction(prob.GetCostFunction(k));
                SetEqualityConstraints(prob.GetEqualityConstraints().at(k).begin(), prob.GetEqualityConstraints().at(k).end());
                SetInequalityConstraints(prob.GetInequalityConstraints().at(k).begin(), prob.GetInequalityConstraints().at(k).end());
                Init();
            }
            ...
            {
                ALTRO_ASSERT(
                    0 <= i && i < NumConstraintFunctions<ConType>(),
                    fmt::format("Invalid constraint index. Got {}, expected to be in range [{},{})", i, 0, NumConstraintFunctions<ConType>())
                );
            ...
            void Hessian(const VectorXdRef& x, const VectorXdRef& u, Eigen::Ref<MatrixXd> dxdx, Eigen::Ref<MatrixXd> dxdu, Eigen::Ref<MatrixXd> dudu) override
            {
                ALTRO_ASSERT(costfun_ != nullptr, "Cost function must be set before evaluating.");
        ...
        private:
            ...
            )
            {
                ...
                {
                    convals->emplace_back(std::make_shared<constraints::ConstraintValues<n, m, ConType>>(this->n_, this->m_, *it));
                }
            }
