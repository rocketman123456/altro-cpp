...
namespace altro
{
    namespace ilqr
    {
        ...
        public:
            ...
            {
                ALTRO_ASSERT(0 <= k_start && k_start <= N_, fmt::format("Start index must be in the interval [0,{}]", N_));
                ALTRO_ASSERT(0 <= k_stop && k_stop <= N_ + 1, fmt::format("Start index must be in the interval [0,{}]", N_ + 1));
                ...
                for (int k = k_start; k < k_stop; ++k)
                {
                    if (n != Eigen::Dynamic)
                    {
                        ALTRO_ASSERT(
                            ...
                            fmt::format("Inconsistent state dimension at knot point {}. Expected {}, got {}", k, n, prob.GetDynamics(k)->StateDimension())
                        );
                    }
                    if (m != Eigen::Dynamic)
                    {
                        ALTRO_ASSERT(
                            ...
                            fmt::format("Inconsistent control dimension at knot point {}. Expected {}, got {}", k, m, prob.GetDynamics(k)->ControlDimension())
                        );
                    }
            ...
            void InitializeFromProblem(const problem::Problem& prob)
            {
                ALTRO_ASSERT(
                    prob.NumSegments() == N_,
                    fmt::format(
                        "Number of segments in problem {}, should be equal to the number of "
            ...
            void SetConstraintCallback(const std::function<double()>& max_violation) { max_violation_callback_ = max_violation; }
            ...
            {
                ALTRO_ASSERT(work_inds_.back() == NumSegments() + 1, "Work inds should include the terminal index.");
            ...
            void Rollout()
            {
                ...
                for (int k = 0; k < N_; ++k)
                {
                    knotpoints_[k]->Dynamics(Z_->State(k), Z_->Control(k), Z_->GetTime(k), Z_->GetStep(k), Z_->State(k + 1));
                }
            }
            ...
            bool RolloutClosedLoop(const double alpha)
            {
                ...
                for (int k = 0; k < N_; ++k)
                {
                    ...
                    GetKnotPointFunction(k).Dynamics(Zbar_->State(k), Zbar_->Control(k), Zbar_->GetTime(k), Zbar_->GetStep(k), Zbar_->State(k + 1));
        ...
        private:
            ...
            void DefaultTaskAssignment()
            {
                ...
                ALTRO_ASSERT(work_inds_.back() == NumSegments(), "Work inds should include the terminal index.");
            ...
            std::vector<std::unique_ptr<KnotPointFunctions<n, m>>> knotpoints_; // problem description and data
