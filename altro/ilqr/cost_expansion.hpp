    ...
    namespace ilqr
    {
        ...
        public:
            ...
            CostExpansion& operator=(const CostExpansion& exp)
            {
                ...
                if (m > 0)
                {
                    ALTRO_ASSERT(m == exp.ControlDimension(), "Invalid copy. Control dimension must be consistent.");
                }
            ...
            void CalcExpansion(const std::shared_ptr<problem::CostFunction>& costfun, const KnotPoint<n2, m2>& z)
            {
                CalcExpansion(costfun, z.State(), z.Control());
            }

            void CalcExpansion(const std::shared_ptr<problem::CostFunction>& costfun, const VectorXdRef& x, const VectorXdRef& u)
            {
